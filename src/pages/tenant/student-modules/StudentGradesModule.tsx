...
export function StudentGradesModule({ myStudent, schoolId }: { myStudent: any; schoolId: string }) {
  ...
  const publishedAssessments = useMemo(() => cachedAssessments.filter(a => a.isPublished), [cachedAssessments]);
  ...
  const subjectNameById = useMemo(() => new Map(cachedSubjects.map(s => [s.id, s.name])), [cachedSubjects]);
  ...
  const handleGenerateRC = () => {
    if (myStudent.status !== "ready" || !schoolId) return;
    generateRC({ schoolId, studentId: myStudent.studentId, schoolName });
  };
            ...
            <Card>
              <CardContent className="flex flex-col items-center gap-4 py-12">
                ...
                <p className="text-sm text-muted-foreground">
                  Generate your consolidated report card with grades, rankings, and attendance.
                </p>
                <Button onClick={handleGenerateRC} disabled={rcLoading || myStudent.status !== "ready"}>
